...
def weighted_boxes_fusion(boxes, scores, labels_list, weights=None, iou_threshold=0.55, skip_box_thr=0.0, conf_type='avg', allows_overflow=False):
    '''
    ...
    :param labels_list: list of labels for each model
    :param weights: list of weights for each model. Default: None, which means weight == 1 for each model
    :param iou_threshold: IoU value for boxes to be a match
    :param skip_box_thr: exclude boxes with score lower than this variable
    :param conf_type: how to calculate confidence in weighted boxes. 'avg': average value, 'max': maximum value
    :param allows_overflow: false if we want confidence score not exceed 1.0
    :return: boxes: boxes coordinates (Order of boxes: x1, y1, x2, y2).
    ...
    if len(weights) != len(boxes):
        print('Warning: incorrect number of weights {}. Must be: {}. Set weights equal to 1.'.format(len(weights), len(boxes)))
        weights = np.ones(len(boxes))
    weights = np.array(weights)
    ...
    filtered_boxes = prefilter_boxes(boxes, scores, labels_list, weights, skip_box_thr)
    if len(filtered_boxes) == 0:
        return np.zeros((0, 4)), np.zeros((0,)), np.zeros((0,))
